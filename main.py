...
# --------------- 插件主类 ---------------
@register("DailyWife", "jmt059", "每日老婆插件", "v1.0.2-fixed", "https://github.com/jmt059/DailyWife")
class DailyWifePlugin(Star):
        ...
        try:
            ...
            for group_id in list(self.pair_data.keys()):
                ...
                if isinstance(pairs, dict) and all(isinstance(v, str) for v in pairs.values()):
                    ...
                    # 确保每个 entry 有 is_initiator 字段
                    for uid, info in pairs.items():
                        if isinstance(info, dict) and "is_initiator" not in info:
            ...
            if COOLING_DATA_PATH.exists():
                with open(COOLING_DATA_PATH, "r", encoding="utf-8") as f:
                    ...
                    for k, v in data.items():
                        ...
                        expire = v.get("expire_time")
                        try:
                            expire_dt = datetime.fromisoformat(expire) if expire else datetime.now()
                        except Exception:
                            expire_dt = datetime.now()
                        cleaned[k] = {"users": users, "expire_time": expire_dt}
                    return cleaned
            return {}
        except Exception as e:
    ...
    def _save_cooling_data(self):
        try:
            ...
            print(f"保存冷静期数据失败: {traceback.format_exc()}")
    ...
    def _is_block_between(self, requester: str, candidate: str, group_id: str) -> bool:
        """
        改进的屏蔽逻辑：
         - GLOBAL_EXCLUDE_QQ 永远排除
         - 如果 requester 主动屏蔽 candidate（scope 匹配），则认为被阻挡
         - 如果 candidate 对 requester 存在且 two_way==True 的条目（candidate 拒绝双向匹配），则认为被阻挡
         - 单向 candidate->requester 的条目（two_way==False）不会阻止 requester 抽到 candidate（只是候选者个人的可见偏好）
        """
        ...
        # 检查 requester 的黑名单（请求者主动屏蔽候选人）
        for e in self.manual_blacklist.get(requester, []):
            scope = str(e.get("scope", "all"))
            if e["blocked_user"] == candidate and (scope == "all" or scope == group_id):
                return True

        # 检查候选者是否明确拒绝与 requester 双向匹配
        for e in self.manual_blacklist.get(candidate, []):
            scope = str(e.get("scope", "all"))
            if e["blocked_user"] == requester and (scope == "all" or scope == group_id) and e.get("two_way", True):
                return True
    ...
    def _is_advanced_enabled(self, group_id: str) -> bool:
        if self.enable_advanced_globally:
            return True
        return bool(self.advanced_enabled.get(group_id, False))

    # --------------- 帮助/复用方法 ---------------
    def _make_avatar_bytes(self, avatar_url: str) -> Optional[Image]:
        # 由于不能在同步里执行 aio 操作，这只是占位，实际调用处使用异步下载逻辑
        return None

    async def _download_avatar_image(self, avatar_url: str) -> Optional[Image]:
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(avatar_url, timeout=10) as resp:
                    if resp.status == 200 and 'image' in resp.headers.get('Content-Type', ''):
                        image_data = await resp.read()
                        return Image.fromBytes(image_data)
        except Exception:
            pass
        return None

    def _sorted_pair_key(self, a: str, b: str) -> str:
        a_s, b_s = str(a), str(b)
        return "-".join(sorted([a_s, b_s]))

    def _is_in_cooling_period(self, user1: str, user2: str) -> bool:
        """
        修复后的冷静期判断逻辑：
         - 如果存在一条冷静期包含两个用户且未过期 -> 匹配
         - 如果存在一条单人 block（例如 block_{user}）且未过期 -> 当请求者或候选人包含该用户时视为冷静期
        """
        now = datetime.now()
        s = {str(user1), str(user2)}
        for pair in self.cooling_data.values():
            users = set(map(str, pair.get("users", [])))
            expire = pair.get("expire_time")
            if not expire or now >= expire:
                continue
            # 完全匹配双方
            if users == s:
                return True
            # 单人 block（例如 block_xxx），当其中一个用户被 block 时也视为在冷静期
            if len(users) == 1 and (str(user1) in users or str(user2) in users):
                return True
        return False
    ...
    async def daily_wife_command(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if user_id in group_data.get("pairs", {}):
                await self._send_partner_message(event, group_id, user_id)
                return
            ...
            # 创建双向配对，同时设置 is_initiator 标记（请求者为 initiator）
            sender_display = self._format_display_info(f"{event.get_sender_name()}({user_id})")
            group_data = self.pair_data.setdefault(group_id, {"date": datetime.now().strftime("%Y-%m-%d"), "pairs": {}, "used": []})
            group_data["pairs"][user_id] = {"user_id": target.user_id, "display_name": target.display_info, "is_initiator": True}
            group_data["pairs"][target.user_id] = {"user_id": user_id, "display_name": sender_display, "is_initiator": False}
            ...
            if self.config.get("show_avatar", True):
                ...
                image_to_send = await self._download_avatar_image(avatar_url)
                if image_to_send:
    ...
    async def _send_partner_message(self, event: AstrMessageEvent, group_id: str, user_id: str):
        """封装：发送已有伴侣信息（查询的公共逻辑）"""
        try:
            group_id = str(group_id)
            user_id = str(user_id)
            ...
            partner_info = group_data["pairs"].get(user_id)
            if not partner_info:
            ...
            print(f"获取/发送伴侣信息异常: {traceback.format_exc()}")
    ...
    async def divorce_command(self, event: AstrMessageEvent):
        try:
            ...
            user_id = str(event.get_sender_id())
            self._check_reset(group_id)
            if group_id not in self.pair_data or user_id not in self.pair_data[group_id].get("pairs", {}):
                yield event.plain_result("🌸 您还没有伴侣哦~")
                return
            partner_info = self.pair_data[group_id]["pairs"][user_id]
            partner_id = str(partner_info["user_id"])

            today = datetime.now().strftime("%Y-%m-%d")
            user_counts = self.breakup_counts.get(today, {})
            current_count = user_counts.get(user_id, 0)
            if current_count >= self.config.get("max_daily_breakups", 3):
                block_hours = self.config.get("breakup_block_hours", 24)
            ...
            group_data["used"] = [uid for uid in group_data.get("used", []) if uid != user_id and uid != partner_id]
            self._save_pair_data()

            cooling_key = self._sorted_pair_key(user_id, partner_id)
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if response_data.get("status") == "failed" and "不存在" in response_data.get("message", ""):
                            ...
                            # 在许愿里，请求者为 initiator，目标为被指定者（is_initiator=False）
                            group_data = self.pair_data.setdefault(group_id, {"date": datetime.now().strftime("%Y-%m-%d"), "pairs": {}, "used": []})
                            group_data["pairs"][user_id] = {"user_id": target_qq, "display_name": f"{target_nickname}({target_qq})", "is_initiator": True}
                            group_data["pairs"][target_qq] = {"user_id": user_id, "display_name": f"{sender_nickname}({user_id})", "is_initiator": False}
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(
                        ...
                        if response_data.get("status") == "failed" and "不存在" in response_data.get("message", ""):
                            ...
                            original_partner_name = self._format_display_info(original_partner_info.get('display_name', '原配'))

                            # 删除被抢夺者及其原配偶的双向记录
                            if target_qq in group_data["pairs"]:
                                orig_id = group_data["pairs"][target_qq]["user_id"]
                                del group_data["pairs"][target_qq]
                                if orig_id in group_data["pairs"] and group_data["pairs"][orig_id]["user_id"] == target_qq:
                                    del group_data["pairs"][orig_id]

                            sender_nickname = event.get_sender_name()
                            # 抢夺后，新配对：请求者 initiator True，被抢者 initiator False
                            group_data["pairs"][user_id] = {"user_id": target_qq, "display_name": f"{target_nickname}({target_qq})", "is_initiator": True}
                            group_data["pairs"][target_qq] = {"user_id": user_id, "display_name": f"{sender_nickname}({user_id})", "is_initiator": False}
    ...
    async def lock_command(self, event: AstrMessageEvent):
        ...
        # 只有被抽方（is_initiator == False）才能锁定
        if pair_info.get("is_initiator", True):
    ...
    def _clean_invalid_cooling_records(self):
        try:
            now = datetime.now()
            expired_keys = [k for k, v in list(self.cooling_data.items()) if v["expire_time"] < now]
